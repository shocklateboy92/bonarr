...
class TransmissionRPCClient {
  ...
  async getTorrents(): Promise<Torrent[]> {
    ...
    const response: TransmissionResponse = await this.makeRequest(
      "torrent-get",
      {
        fields,
        format: "objects",
    ...
    return response.arguments.torrents.sort(
  ...
  async getTorrentFiles(torrentId: number): Promise<TorrentWithFiles> {
    ...
    if (
      ...
      response.arguments.torrents.length === 0
    ) {
  ...
  async addTorrent(
    ...
    return (
      ...
      response.arguments["torrent-duplicate"] ||
      {}
    );
  }
